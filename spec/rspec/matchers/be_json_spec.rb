...
module Rspec
  module Matchers
    ...
    describe "actual.should be_json_including(expected)" do
      ...
      context "a json string against a regex" do
        ...
        let(:failing)  { '{ "x" : {"a" : "123"}}'      }
        let(:failure_message) {
          'No match for {"x"=>{"a"=>/[A-Z]{3}/}}'
        }

				it "passes if include" do
					actual.should be_json_including expected
				end

				it "fails if doesn't include" do
					lambda { failing.should be_json_including expected }.should fail_with failure_message
				end
      end
    end
  end
