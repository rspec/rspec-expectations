      ...
      class RaiseError
        ...
        def matches?(given_proc, negative_expectation = false)
          if negative_expectation && (expecting_specific_exception? || @expected_message)
            parameters = if expecting_specific_exception? && @expected_message
                           "SpecificErrorClass, message"
                         elsif expecting_specific_exception?
                           "SpecificErrorClass"
                         elsif @expected_message
                           "message"
                         end
            what_to_deprecate = "`expect { }.not_to raise_error(#{parameters})`"
            replacement = [
              "`expect { }.not_to raise_error` (with no args) or ",
              "`expect { }.to raise_error(#{parameters})`"
            ].join

            RSpec.deprecate(what_to_deprecate, :replacement => replacement)
          end
